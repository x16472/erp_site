...
如果像是員工主檔，在呈現到前端的部分預設以員工編號進行排序，部門則作為可選的排序方式。輸出到前端前必須由`data.py`先做，這樣才能減小`app.py`的壓力。
...
- 員工入口驗證與打卡整合：員工編號通過 SQL Server 在職驗證且打卡成功後，才建立 HttpOnly 工作階段；內部與管理 API 會再次確認員工仍在職。
